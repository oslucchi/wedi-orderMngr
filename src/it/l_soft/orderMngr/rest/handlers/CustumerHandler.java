...
public class CustumerHandler {
	...
	public Response updateCustomer(String body, @PathParam("id") int idOrder, @HeaderParam("Language") String language)
	{
		int languageId = Utils.setLanguageId(language);
		
		JsonObject jsonIn = JavaJSONMapper.StringToJSON(body);
		Customers customer = (Customers) JavaJSONMapper.JSONToJava(jsonIn.getJsonObject("customer"), Customers.class);
		try 
		{
			conn = DBInterface.connect();
			DBInterface.TransactionStart(conn);
			customer.update(conn, "idCustomers");
